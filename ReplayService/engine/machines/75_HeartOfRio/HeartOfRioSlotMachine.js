...
SlotMachine.prototype.SpinForFreeGen = function (bpl, totalBet, fsWin, isCall = false) {
    var freeSpinCacheList = [], moneyCacheList = [];
    ...
    var pattern = {
        ...
        moneyCache: moneyCacheList.concat(freeCache.moneyCache),
        bpl: bpl,
        type: "FREE",
        isCall: isCall ? 1 : 0
    };
...
var RandomScatterView = function (reels, bpl) {
    ...
    while (true) {
        view = GetBaseView(reels, bpl);

        if (isFreeSpinWin(view.view)) {
            break;
        }
    }
...
var RandomFreeViewCache = function (reels, bpl, fsWin, fsLen) {
    ...
    for (var patternIndex = 0; patternIndex < 200; patternIndex++) {
        ...
        while (true) {
            var result = GetFreeView(bpl, freeSpinBonusPot);
...
var GetFreeView = function (bpl, fsBonusPot) {
    while (true) {
        var view = RandomView(freeReels);
        var win = WinFromView(view, bpl);
