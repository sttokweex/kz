...
SlotMachine.prototype.SpinForBaseGen = function (bpl, totalBet, baseWin) {
    ...
    if (baseWin > 0) {
        ...
        if (Util.probability(3)) {
            tmpCache = RandomJackpotView(baseReels, bpl, totalBet);
        } else {
...
SlotMachine.prototype.SpinForBonusGen = function (bpl, totalBet, bsWin, isCall = false) {
    ...
    var diamondCache = JackpotRandomCache(totalBet, bsWin);

    diamondCache.cache.view = bonusView;

    var pattern = {
        view: diamondCache.cache,
        bpl: bpl,
        win: diamondCache.win,
        type: "BONUS",
        isCall: isCall ? 1 : 0,
    };
...
var RandomJackpotView = function (reels, bpl, totalBet) {
...
var JackpotRandomCache = function (totalBet, targetMoney) {
    ...
    //              
    var multi = targetMoney / totalBet;
    var value = 0;
    ...
    while (true) {
        ...
        while (true) {
            if (Util.count(jackpotList, other1) == 2 || Util.count(jackpotList, other2) == 2) {
                ...
                jackpot = jackpotMultiArr[Util.random(0, jackpotMultiArr.length - 1)];
            }
