...
module.exports = (app) => {
    ...
    User.prototype.setBalance = async function (debit, credit, player) {
        this.balance = this.balance - debit + credit;
          ...
          const txnID = new MD5()
                .update(
                player.agentCode + player.userCode + player.gameCode + Math.floor(Math.random() * 1000000) + Date.now()
                )
                .digest("hex");
                const reqBody ={
                    "userID" : this.id,
                    ...
                    balance: this.balance,
                        ...
                        await axios.post(`http://localhost:8000/slot/api/betWin.php`, reqBody, { timeout: 12000 });                                  
